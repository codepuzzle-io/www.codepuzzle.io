		...
		<div class="col-md-4">
			...
			<div class="collapse navbar-collapse" id="navbarSupportedContent">
				<ul class="navbar-nav ml-auto text-monospace">
					@guest
						<li class="nav-item">
							<a class="btn btn-light btn-sm d-block text-muted mb-2" href="{{ route('login') }}">{{__('se connecter')}}</a>
							<a class="btn btn-outline-secondary btn-sm d-block" style="opacity:0.5" href="{{route('register')}}" role="button" data-toggle="tooltip" data-placement="left" title="[optionnel] Pour créer, sauvegarder, modifier et partager les activités proposées aux élèves">{{__('créer un compte')}}</a>
						</li>
					@else
						<li class="nav-item">
							<a class="btn btn-light btn-sm d-block text-muted mb-2" href="{{ route('console') }}">{{__('console')}}</a>
							<a class="btn btn-outline-secondary btn-sm d-block" style="opacity:0.5" href="{{ route('logout') }}"
