...
$copies_locked = App\Models\Copie::where([['jeton_devoir', $devoir->jeton], ['locked', 1]])->exists();
?>
<!doctype html>
<html lang="fr">
<head>
    ...
    <title>DEVOIR | {{$devoir->jeton}} | SUPERVISION</title>
    <style>
        .hljs {
            padding:0px;
            font-size:10px;
                            ...
                            <div class="col mb-4">
                                <div class="card p-0 h-100">
                                    <div class="card-body p-0">
                                        <div class="text-monospace small">{{$copie->pseudo}}</div>
                                        @if($copie->locked == 0)
                                            <div class="h-100 text-white text-center" style="background-color:silver;min-height:60px;border-radius:3px;display:flex;justify-content:center;align-items:center;">                                  
                                                <a class="btn btn-secondary btn-sm pl-4 pr-4 text-monospace small" href="/copie-eleve/{{Crypt::encryptString($copie->id)}}" role="button">voir la copie</a>
                                            </div>
                                        @else
                                            <div class="h-100 text-white text-center" style="background-color:#e3413d;min-height:60px;border-radius:3px;display:flex;justify-content:center;align-items:center;">                                  
                                                <button type="button" onclick="deverrouiller('{{$copie->jeton_copie}}')" class="btn btn-danger btn-sm pl-4 pr-4 text-monospace small">déverrouiller</button>
                                            </div>
                                        @endif
                                    </div>
                                </div>
